...
9. Loads historical zone demand automatically and generates forecasts directly in the live dashboard.
...
The Streamlit application presents headline metrics, horizon comparisons, calibrated intervals, and zone level error analysis. Its interactive simulator lets a user select a taxi zone, forecast horizon, and target time. The application automatically calculates recent demand features from its deployed TLC history snapshot, then returns expected pickups, change from recent demand, and capacity planning scenarios. A custom mode remains available for what if analysis.
